...
_redirect_to_shard(struct sqlx_sqlite3_s *sq3, const gchar *path)
{
	...
	// Share some properties with the shard container
...
static GError *
_check_shard_range_or_redirect_to_shard(struct sqlx_sqlite3_s *sq3,
		struct oio_url_s *url, gboolean read_only)
{
	GError *err = NULL;
	const gchar *path = oio_url_get(url, OIOURL_PATH);
	if (oio_ext_is_shard())
		err = _check_shard_range(sq3, url, path, read_only);
	else
		...
		m2_onbean_cb cb, gpointer u0)
{
	...
	if (!err) {
		err = _check_shard_range_or_redirect_to_shard(sq3, url, TRUE);
	}
	if (!err) {
		...
		struct oio_url_s *url, m2_onbean_cb cb, gpointer u0)
{
	...
	err = m2b_open(m2, url, M2V2_OPEN_MASTERONLY | M2V2_OPEN_ENABLED, &sq3);
	if (!err) {
		err = _check_shard_range_or_redirect_to_shard(sq3, url, FALSE);
	}
	if (!err) {
		EXTRA_ASSERT(sq3 != NULL);
		if (!(err = sqlx_transaction_begin(sq3, &repctx))) {
...
__copy_base(struct sqlx_sqlite3_s *sq3 UNUSED, gchar *dst)
{
	...
	if (rc != 0) {
		if (errno == EOPNOTSUPP) {
			GRID_ERROR("Reflink is not enabled, "
		...
		struct oio_url_s *url, gint64 *ptimestamp)
{
	...
	err = m2b_open(m2b, url, M2V2_OPEN_MASTERONLY|M2V2_OPEN_ENABLED, &sq3);
	if (!err) {
		*ptimestamp = oio_ext_real_time();

		struct sqlx_repctx_s *repctx = NULL;
		if (!(err = _transaction_begin(sq3, url, &repctx))) {
			gchar *dst = g_strdup_printf("%s.sharding-%"G_GINT64_FORMAT,
					sq3->path_inline, *ptimestamp);
			err = __copy_base(sq3, dst);
			err = sqlx_transaction_end(repctx, err);
			if (!err)
				m2b_add_modified_container(m2b, sq3);
			g_free(dst);
...
meta2_backend_replace_container_sharding(struct meta2_backend_s *m2b,
		struct oio_url_s *url, gchar *shards_str)
{
	GError *err = NULL;
	struct sqlx_sqlite3_s *sq3 = NULL;
	shards_container_t shards = NULL;
